...
import com.lightbend.lagom.javadsl.internal.persistence.PersistentEntityRegistryDelegate
import com.lightbend.lagom.javadsl.internal.persistence.cassandra.CassandraSessionDelegate
import com.lightbend.lagom.javadsl.persistence.cassandra.CassandraSession
...
class PersistenceModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    binder.bind(classOf[CoreCassandraSession]).to(classOf[InternalCassandraSession])
    binder.bind(classOf[CassandraSession]).to(classOf[CassandraSessionDelegate])
